...
class RefCodeGeneration:
    ...
    def __init__(self, cose_service_cls=CodesRepository, users_service_cls=UsersRepository):
    ...
    async def generate_code(self, user_id, length=10):
        ...
        try:
            new_code = await self._code_service.create_code(code=code, expired_date=expired_date, user_id=user_id)
            return new_code
        except:
