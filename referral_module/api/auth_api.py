...
async def register(
    ...
    try:
        new_user = await registration_func(
            form_data.username, form_data.password, ref_code
        )
...
async def login(
    form_data: Annotated[OAuth2PasswordRequestForm, Depends()],
    user_service=Depends(get_auth_user_func),
):
    try:
...
async def auth_google(
    ...
    try:
        ...
        raise HTTPException(status_code=HTTPStatus.UNAUTHORIZED, detail=exc.args)
